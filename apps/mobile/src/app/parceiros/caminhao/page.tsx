...
export default function CaminhaoDashboard() {
  ...
  const corridasDisponiveis = store.orders.filter(o => o.status === 'pronto' && o.motoristaId === null && (o.type === 'B2B' || o.type === 'COLETA') && store.users[o.origemId]?.cidade === currentUser.cidade);
                ...
                <div className="space-y-4">
                  {minhasCorridas.length === 0 ? (
                    ...
                    <div key={o.id} className={`bg-white dark:bg-zinc-900 p-4 rounded-xl shadow-sm border ${o.status === 'em_rota' ? 'border-blue-400 dark:border-blue-600' : 'border-zinc-200 dark:border-zinc-800'}`}>
                        ...
                        ) : o.status === 'em_rota' && o.confirmacao.entregador && !o.confirmacao.recebedor ? (
                            <div className="bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-800 text-xs p-2 rounded text-center font-bold">Aguardando confirmação do local...</div>
                        ) : (
